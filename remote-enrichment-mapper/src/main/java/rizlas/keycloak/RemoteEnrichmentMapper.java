...
public class RemoteEnrichmentMapper extends AbstractOIDCProtocolMapper
    ...
    static {
        configProperties.add(new ProviderConfigProperty(
                URL_PROPERTY_NAME,
                "URL",
                "HTTP(S) URL of the authz enrichment endpoint that will be called during token issuance.",
                ProviderConfigProperty.STRING_TYPE,
                URL_PROPERTY_DEFAULT,
                false,
                true));
        configProperties.add(new ProviderConfigProperty(
                URL_AUTH_TOKEN_PROPERTY_NAME,
                "Authentication Token",
                "Token used to authenticate requests to the endpoint. Leave empty if it does not require authentication.",
                ProviderConfigProperty.PASSWORD,
                URL_AUTH_TOKEN_PROPERTY_DEFAULT,
                true,
                false));
        configProperties.add(new ProviderConfigProperty(
                "tokenClaimNameInfo",
                ...
                """
                        When Token Claim Name is set, stores the entire authorization response in one claim.
                        When empty, the mapper emits individual claims such as quota, groups, etc..
                        """,
                ProviderConfigProperty.STRING_TYPE,
